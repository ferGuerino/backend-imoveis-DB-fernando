...
describe("POST /realEstate", () => {
  ...
  it("Success: Must be able to create real estates - Admin token - Full body", async () => {
    const { categoryToCreate, ...payload } = createRealEstateRouteMock.realEstateComplete;

    const category = await AppDataSource.getRepository(Category).save(categoryToCreate);
  ...
  it("Error: Must not be able to create real estates - Admin token - Unique address", async () => {
    const { categoryToCreate, ...payload } = createRealEstateRouteMock.realEstateUnique;
  ...
  it("Error: Must not be able to create real estates - Admin token - Invalid body", async () => {
    const { categoryToCreate, ...realEstateInfo } = createRealEstateRouteMock.realEstateInvalidBody;
  ...
  it("Error: Must not be able to create real estates - Admin token - Invalid body 2", async () => {
    const { categoryToCreate, ...realEstateInfo } = createRealEstateRouteMock.realEstateInvalidBody2;
